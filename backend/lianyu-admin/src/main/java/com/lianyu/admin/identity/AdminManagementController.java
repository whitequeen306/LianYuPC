...
public class AdminManagementController {
    ...
    @PostMapping("/{id}/revoke-sessions") public Result<Void> revokeSessions(@PathVariable long id) { authorization.require("admin.manage"); jdbc.update("UPDATE admin_session SET revoked_at=CURRENT_TIMESTAMP(3) WHERE admin_user_id=? AND revoked_at IS NULL", id); audit.record("admin.session.revoke", "admin_user", String.valueOf(id), "success", null); return Result.ok(); }
