export default {
  ...
  about: {
    ...
    mcpCommand: 'エンジンコマンド',
    mcpCommandPlaceholder: 'パッケージ版は exe パス（例 C:\\AgentAssistant\\engine.exe）、ソース実行は python',
    mcpCommandHint: '空欄ならダウンロード済みの AgentEngine を使います。カスタム時は実行ファイルパスを指定し、ソース実行では作業ディレクトリを AgentAssistant のルートにしてください。',
    mcpArgs: '起動引数（1 行に 1 つ）',
    mcpArgsPlaceholder: 'ソース例：\n-m\nagent_assistant.hosted.mcp_server',
    mcpArgsHint: '1 行に 1 引数。パッケージ版は通常空欄、ソース実行は -m とモジュール名の 2 行。',
    mcpCwd: '作業ディレクトリ（任意）',
    mcpCwdPlaceholder: '例：C:\\Users\\hp\\Desktop\\AgentAssistant',
    mcpDisabledHint: '有効にするとローカル AgentEngine（約 60MB）をダウンロードして起動し、そのツールをクラウドのキャラクターに登録します。危険な操作は必ず確認します。',
    mcpStateRunning: '実行中',
