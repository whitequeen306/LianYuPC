export default {
  ...
  qqBridge: {
    ...
    status: { title: '运行状态' },
    binding: { title: '消息路由', character: 'AI 角色', characterPlaceholder: '选择 AI 角色', characterEmpty: '暂无角色，请先在 YuNian App 内创建', characterHint: '选定角色后，桥接为每个 QQ 用户自动匹配该角色的专属会话（无需手填会话号）；清空上下文后会话失效时自动重建，不受影响。', provider: '文本 Provider', providerPlaceholder: '请选择已配置的文本 Provider', providerHint: '必须选择自己的文本 Provider；平台内置聊天模型已下线。', providerRequired: '请先选择文本 Provider（须在设置中添加配置）', model: '文本模型', modelPlaceholder: '选择或填写模型名', modelHint: '留空则用该 Provider 的默认模型。', conversationId: '云端会话', conversationIdHint: '首次启动桥接时会自动绑定最近的单聊会话（全自动）；如需改用其他 AI 角色，可在上方下拉中选择。', convPlaceholder: '选择云端会话', convEmpty: '暂无云端会话', convCreate: '立即创建默认会话', convCreateOk: '已创建并绑定默认会话', convCreateFail: '创建会话失败，请稍后重试', noCharacter: '请先在 YuNian App 内创建角色', currentlyBound: '当前绑定', singleFallback: '单聊 #{id}', allowMode: '放行模式', allowModeAllowlist: '白名单（默认拒绝）', allowModeOpen: '开放（任何人）', openModeWarning: '⚠️ 开放模式会导致 AI 认错人、上下文错乱', openModeWarningDesc: '任何人都能触发 AI 回复，不同 QQ 用户的消息会混在一起，AI 可能认错人、记错上下文。强烈建议改回白名单模式，在下方填入你自己的 QQ 号（私聊白名单）和群号（群白名单），多个用英文逗号分隔。', emptyAllowlistWarning: '白名单为空：当前没有 QQ 消息会被转发到 AI。在下方添加 QQ 号/群号以放行。', allowUsers: '私聊白名单（QQ号）', allowGroups: '群白名单（群号）', allowHint: '白名单模式下留空 = 不放行任何人；多个用英文逗号分隔', save: '保存路由', saved: '路由已保存', pickByCharacter: '按角色名获取会话', pickCharacterTitle: '选择 AI 角色', pickCharacterHint: '查找或新建该角色的单聊会话并绑定', pickConfirm: '获取并绑定', pickOk: '已绑定该角色的会话', pickFail: '获取会话失败，请稍后重试' },
    ...
    reason: { not_auto_mode: '请先在上方开启自动托管', not_consented: '请先完成 QQ 托管授权', start_failed: '启动失败，请检查网络后重试', reinstall_failed: '重装失败，请重试或检查网络', not_running: '运行时未就绪，请先启动托管', no_conversation: '请先在下方「消息路由」选择会话', not_logged_in: '请先登录予念账号', disabled: '桥接已被禁用', qqnt_not_found: '未检测到 QQ 桌面版（QQNT），请先安装后再启用自动托管：https://im.qq.com/index/', exception: '操作异常，请重试', unknown: '操作失败，请重试' }
  },
  wechatBridge: {
    desc: '用手机微信里的 ClawBot 与已绑定的予念角色聊天。电脑微信常常看不到入口，请用手机扫码。本机不安装 Node，也不运行 OpenClaw 自带大脑。',
    notElectron: '微信 ClawBot 通道仅在桌面客户端可用。',
    consent: {
      title: '开启微信 ClawBot',
      body: '应用会下载予念打包的本机通道到本机并启动。请用自己的微信扫描 ClawBot 登录；凭证只保存在本机。回复来自你绑定的予念角色会话，而不是 OpenClaw 自带 agent。',
      confirm: '我已了解，开启',
      cancel: '取消'
    },
    status: {
  ...
  auth: {
    ...
    brandRegister: '创建你的 YuNian 账号，开启你的故事',
    welcomeBack: '欢迎回来',
    loginDesc: '登录你的 YuNian 账号继续对话',
    registerTitle: '注册账号',
    registerBrand: '初次相遇',
    username: '用户名',
    password: '密码',
  ...
  characters: {
    ...
    import: '导入角色',
    dialogImport: '导入角色',
    importHint: '上传人设卡或聊天记录，用你自己的模型抽取性格后创建。不会写入聊天记录。',
    importSource: '人设或聊天记录',
    importSourcePlaceholder: '粘贴人设，或微信/QQ/角色卡导出的聊天记录……',
    importSourceHint: '支持 txt / md / json / html，最大 2MB。人设和聊天记录都只用来抽取性格。',
    importPickFile: '选择文件',
    importAvatar: '角色头像',
    importChangeAvatar: '更换头像',
    importPickAvatar: '选择图片',
    importAvatarHint: 'JPG/PNG/WebP · 最大 8MB',
    cancel: '取消',
    importAddressing: '最常用称呼',
    importAddressingPlaceholder: '例如：你、笨蛋、指挥官',
    importAddressingHint: '角色平时最常怎么叫你。这是口吻参考，不是每句都必须这样喊。',
    importNeedSource: '请上传或粘贴人设 / 聊天记录',
    importNeedAddressing: '请填写角色最常用的称呼',
    importNeedModel: '未配置文本模型，请先在设置中添加',
    importFileType: '请选择文本文件（txt / md / json / html）',
    importFileTooLarge: '文件不能超过 2MB',
    importAnalyzing: '正在抽取角色设定…',
    importSuccess: '已导入角色「{name}」'
  },
  cityMode: {
  ...
  settings: {
    ...
    aiSection: 'AI 接口配置',
    aiDesc: '必须配置自己的接口地址、API Key 与模型后才能聊天；平台内置文本模型已下线',
    addConfig: '添加配置',
    emptyTitle: '尚未配置文本模型',
    emptyDesc: '尚未配置文本模型。请添加至少一套配置后才能聊天、群聊与 QQ 桥接',
    addFirst: '添加第一套配置'
  },
  about: {
    title: '关于予念',
    ...
    envWeb: '网页版',
    envDesktop: '桌面版',
    intro: '予念（YuNian）是一款多角色 AI 陪伴应用，支持角色对话、朋友圈、日记、记忆与桌面桌宠。可在 Windows 桌面端使用，也能通过 QQ 桥接（beta）把消息转发给云端 AI。',
    techStack: '技术栈',
    techStackDesc: '前端 Vue 3 + Element Plus + Electron；后端 Spring Boot + MyBatis-Plus + MySQL。',
    copyright: '© 2026 予念 YuNian',
    backToSettings: '返回设置',
    mcpTitle: 'MCP 服务（实验性）',
    mcpAgentAssistantDesc: '本地电脑助手：开启后角色可以在聊天中调用你电脑上的工具（打开应用、整理文件等）。危险操作会先弹窗征求你的确认。',
    mcpBridge: '云端桥接',
    mcpBridgeOnline: '已连接（角色可调用本地工具）',
    mcpBridgeOffline: '未连接',
    mcpTools: '可用工具',
    mcpEngine: '引擎',
    mcpEngineOfficial: '官方 AgentEngine',
    mcpEngineLocal: '本地源码',
    mcpEngineOfficialHint: '使用予念云端分发的官方引擎，开启电脑操控时自动下载更新。',
    mcpEngineLocalHint: '用本机 AgentAssistant 仓库跑引擎，改完源码重启电脑操控即可测。',
    mcpEngineLocalRunning: '本地源码（{command}）',
    mcpEngineManaged: 'AgentEngine {version}',
    mcpEngineNotInstalled: '尚未安装（开启时将从云端下载）',
    ...
    mcpCommand: 'Python 解释器',
    mcpCommandPlaceholder: '例如 C:\\Users\\hp\\AppData\\Local\\Programs\\Python\\Python311\\python.exe',
    mcpCommandHint: '工作目录填 AgentAssistant 仓库根，例如 C:\\Users\\hp\\Desktop\\AgentAssistant。',
    mcpArgs: '启动参数（每行一个）',
    mcpArgsPlaceholder: '-m\nagent_assistant.hosted.mcp_server',
    mcpArgsHint: '源码方式两行：-m 与 agent_assistant.hosted.mcp_server。',
    mcpCwd: '工作目录',
    mcpCwdPlaceholder: '例如 C:\\Users\\hp\\Desktop\\AgentAssistant',
    mcpDisabledHint: '开启后会下载并启动官方 AgentEngine（约 60MB），把它的工具注册给云端角色。危险操作会先弹窗确认。',
    mcpDisabledHintLocal: '开启后会用上面填写的 Python 启动本机 AgentAssistant 源码。改完源码后关掉再打开电脑操控即可测。',
    mcpStateRunning: '运行中',
    mcpStateStarting: '启动中',
    mcpStateStopped: '已停止',
    mcpStateError: '异常',
    mcpControlBanner: '{name}正在操控你的电脑，按 Esc 取消',
