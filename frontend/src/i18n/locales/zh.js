export default {
  ...
  qqBridge: {
    ...
    status: { title: '运行状态' },
    binding: { title: '消息路由', character: 'AI 角色', characterPlaceholder: '选择 AI 角色', characterEmpty: '暂无角色，请先在 LianYu App 内创建', characterHint: '选定角色后，桥接为每个 QQ 用户自动匹配该角色的专属会话（无需手填会话号）；清空上下文后会话失效时自动重建，不受影响。', provider: '文本 Provider', providerHint: '平台默认走运营共享 Key；选自己的 Provider 后 QQ 请求会用你的配置与模型。', model: '文本模型', modelPlaceholder: '平台默认时无需选择', modelHint: '仅自定义 Provider 可改；留空则用该 Provider 的默认模型。', visionModel: '识图模型', visionModelPlaceholder: '平台默认识图 (qwen3-vl-plus)', visionModelHint: 'QQ 发图时的多模态识图模型；留空走平台默认 qwen3-vl-plus。', conversationId: '云端会话', conversationIdHint: '首次启动桥接时会自动绑定最近的单聊会话（全自动）；如需改用其他 AI 角色，可在上方下拉中选择。', convPlaceholder: '选择云端会话', convEmpty: '暂无云端会话', convCreate: '立即创建默认会话', convCreateOk: '已创建并绑定默认会话', convCreateFail: '创建会话失败，请稍后重试', noCharacter: '请先在 LianYu App 内创建角色', currentlyBound: '当前绑定', singleFallback: '单聊 #{id}', allowMode: '放行模式', allowModeAllowlist: '白名单（默认拒绝）', allowModeOpen: '开放（任何人）', openModeWarning: '⚠️ 开放模式会导致 AI 认错人、上下文错乱', openModeWarningDesc: '任何人都能触发 AI 回复，不同 QQ 用户的消息会混在一起，AI 可能认错人、记错上下文。强烈建议改回白名单模式，在下方填入你自己的 QQ 号（私聊白名单）和群号（群白名单），多个用英文逗号分隔。', emptyAllowlistWarning: '白名单为空：当前没有 QQ 消息会被转发到 AI。在下方添加 QQ 号/群号以放行。', allowUsers: '私聊白名单（QQ号）', allowGroups: '群白名单（群号）', allowHint: '白名单模式下留空 = 不放行任何人；多个用英文逗号分隔', save: '保存路由', saved: '路由已保存', pickByCharacter: '按角色名获取会话', pickCharacterTitle: '选择 AI 角色', pickCharacterHint: '查找或新建该角色的单聊会话并绑定', pickConfirm: '获取并绑定', pickOk: '已绑定该角色的会话', pickFail: '获取会话失败，请稍后重试' },
