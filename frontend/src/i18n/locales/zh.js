export default {
  ...
  about: {
    ...
    mcpUseDemo: '使用内置演示服务',
    mcpCommand: '引擎命令',
    mcpCommandPlaceholder: '打包版填 exe 路径，如 C:\\AgentAssistant\\engine.exe；源码调试填 python',
    mcpCommandHint: '留空则使用已下载的 AgentEngine。自定义时填可执行文件路径；源码方式请把工作目录设为 AgentAssistant 仓库根。',
    mcpArgs: '启动参数（每行一个）',
    mcpArgsPlaceholder: '源码调试示例：\n-m\nagent_assistant.hosted.mcp_server',
    mcpArgsHint: '每行一个参数。打包版通常留空；源码方式填 -m 与模块名两行。',
    mcpCwd: '工作目录（可选）',
    mcpCwdPlaceholder: '例如 C:\\Users\\hp\\Desktop\\AgentAssistant',
    mcpDisabledHint: '开启后会下载并启动本地 AgentEngine（约 60MB），把它的工具注册给云端角色。危险操作会先弹窗确认。',
    mcpStateRunning: '运行中',
