export default {
  ...
  qqBridge: {
    ...
    status: { title: '執行狀態' },
    binding: { title: '訊息路由', character: 'AI 角色', characterPlaceholder: '選擇 AI 角色', characterEmpty: '暫無角色，請先在 YuNian App 內建立', characterHint: '選定角色後，橋接為每個 QQ 使用者自動匹配該角色的專屬會話（無需手填會話號）；清空上下文後會話失效時自動重建，不受影響。', provider: '文字 Provider', providerHint: '平台預設走營運共用 Key；選自己的 Provider 後 QQ 請求會用你的配置與模型。', model: '文字模型', modelPlaceholder: '平台預設時無需選擇', modelHint: '僅自訂 Provider 可改；留空則用該 Provider 的預設模型。', conversationId: '雲端會話', conversationIdHint: '首次啟動橋接時會自動綁定最近的單聊會話（全自動）；如需改用其他 AI 角色，可在上方下拉中選擇。', convPlaceholder: '選擇雲端會話', convEmpty: '暫無雲端會話', convCreate: '立即建立預設會話', convCreateOk: '已建立並綁定預設會話', convCreateFail: '建立會話失敗，請稍後重試', noCharacter: '請先在 YuNian App 內建立角色', currentlyBound: '目前綁定', singleFallback: '單聊 #{id}', allowMode: '放行模式', allowModeAllowlist: '白名單（預設拒絕）', allowModeOpen: '開放（任何人）', openModeWarning: '⚠️ 開放模式會導致 AI 認錯人、上下文錯亂', openModeWarningDesc: '任何人都能觸發 AI 回覆，不同 QQ 使用者的訊息會混在一起，AI 可能認錯人、記錯上下文。強烈建議改回白名單模式，在下方填入你自己的 QQ 號（私聊白名單）和群號（群白名單），多個用英文逗號分隔。', emptyAllowlistWarning: '白名單為空：目前沒有 QQ 訊息會被轉發到 AI。在下方新增 QQ 號/群號以放行。', allowUsers: '私聊白名單（QQ號）', allowGroups: '群白名單（群號）', allowHint: '白名單模式下留空 = 不放行任何人；多個用英文逗號分隔', save: '儲存路由', saved: '路由已儲存', pickByCharacter: '按角色名取得會話', pickCharacterTitle: '選擇 AI 角色', pickCharacterHint: '尋找或新建該角色的單聊會話並綁定', pickConfirm: '取得並綁定', pickOk: '已綁定該角色的會話', pickFail: '取得會話失敗，請稍後重試' },
    ...
    reason: { not_auto_mode: '請先在上方開啟自動託管', not_consented: '請先完成 QQ 託管授權', start_failed: '啟動失敗，請檢查網路後重試', reinstall_failed: '重裝失敗，請重試或檢查網路', not_running: '執行環境未就緒，請先啟動託管', no_conversation: '請先在下方「訊息路由」選擇會話', not_logged_in: '請先登入予念帳號', disabled: '橋接已被停用', qqnt_not_found: '未偵測到 QQ 桌面版（QQNT），請先安裝後再啟用自動託管：https://im.qq.com/index/', exception: '操作異常，請重試', unknown: '操作失敗，請重試' }
  },
  wechatBridge: {
    desc: '用手機微信裡的 ClawBot 與已綁定的予念角色聊天。電腦微信常常看不到入口，請用手機掃碼。本機不安裝 Node，也不執行 OpenClaw 自帶大腦。',
    notElectron: '微信 ClawBot 通道僅在桌面用戶端可用。',
    consent: {
      title: '開啟微信 ClawBot',
      body: '應用會下載予念打包的本機通道到本機並啟動。請用自己的微信掃描 ClawBot 登入；憑證只保存在本機。回覆來自你綁定的予念角色會話，而不是 OpenClaw 自帶 agent。',
      confirm: '我已了解，開啟',
      cancel: '取消'
    },
    status: {
  ...
  auth: {
    ...
    brandRegister: '建立你的 YuNian 帳號，開啟你的故事',
    welcomeBack: '歡迎回來',
    loginDesc: '登入你的 YuNian 帳號繼續對話',
    registerTitle: '註冊帳號',
    registerBrand: '初次相遇',
    username: '使用者名稱',
    password: '密碼',
  ...
  characters: {
    ...
    import: '匯入角色',
    dialogImport: '匯入角色',
    importHint: '上傳人設卡或聊天紀錄，用你自己的模型抽取性格後建立。不會寫入聊天紀錄。',
    importSource: '人設或聊天紀錄',
    importSourcePlaceholder: '貼上人設，或微信/QQ/角色卡匯出的聊天紀錄……',
    importSourceHint: '支援 txt / md / json / html，最大 2MB。人設和聊天紀錄都只用來抽取性格。',
    importPickFile: '選擇檔案',
    importAvatar: '角色頭像',
    importChangeAvatar: '更換頭像',
    importPickAvatar: '選擇圖片',
    importAvatarHint: 'JPG/PNG/WebP · 最大 8MB',
    cancel: '取消',
    importAddressing: '最常用稱呼',
    importAddressingPlaceholder: '例如：你、笨蛋、指揮官',
    importAddressingHint: '角色平時最常怎麼叫你。這是口吻參考，不是每句都必須這樣喊。',
    importNeedSource: '請上傳或貼上人設 / 聊天紀錄',
    importNeedAddressing: '請填寫角色最常用的稱呼',
    importNeedModel: '未設定文本模型，請先在設定中新增',
    importFileType: '請選擇文本檔（txt / md / json / html）',
    importFileTooLarge: '檔案不能超過 2MB',
    importAnalyzing: '正在抽取角色設定…',
    importSuccess: '已匯入角色「{name}」'
  },
  cityMode: {
  ...
  chat: {
    ...
    exportFailed: '匯出失敗',
    mcpTaskActive: '電腦任務進行中',
    mcpTaskStarting: '正在開始執行…',
  },
  group: {
    title: '群聊',
  ...
  about: {
    ...
    envWeb: '網頁版',
    envDesktop: '桌面版',
    intro: '予念（YuNian）是一款多角色 AI 陪伴應用，支援角色對話、朋友圈、日記、記憶與桌面桌寵。可在 Windows 桌面端使用，也能透過 QQ 橋接（beta）把訊息轉發給雲端 AI。',
    techStack: '技術棧',
    techStackDesc: '前端 Vue 3 + Element Plus + Electron；後端 Spring Boot + MyBatis-Plus + MySQL。',
    copyright: '© 2026 予念 YuNian',
    backToSettings: '返回設定',
    mcpTitle: 'MCP 服務（實驗性）',
    mcpAgentAssistantDesc: '本機電腦助手：開啟後角色可以在聊天中調用你電腦上的工具（開啟應用、整理檔案等）。危險操作會先彈窗徵求你的確認。',
    mcpBridge: '雲端橋接',
    mcpBridgeOnline: '已連接（角色可調用本機工具）',
    mcpBridgeOffline: '未連接',
    mcpTools: '可用工具',
    mcpEngine: '引擎',
    mcpEngineOfficial: '官方 AgentEngine',
    mcpEngineLocal: '本機原始碼',
    mcpEngineOfficialHint: '使用予念雲端發佈的官方引擎，開啟電腦操控時自動下載更新。',
    mcpEngineLocalHint: '用本機 AgentAssistant 倉庫跑引擎，改完原始碼重啟電腦操控即可測。',
    mcpEngineLocalRunning: '本機原始碼（{command}）',
    mcpEngineManaged: 'AgentEngine {version}',
    mcpEngineNotInstalled: '尚未安裝（開啟時將從雲端下載）',
    ...
    mcpCommand: 'Python 直譯器',
    mcpCommandPlaceholder: '例如 C:\\Users\\hp\\AppData\\Local\\Programs\\Python\\Python311\\python.exe',
    mcpCommandHint: '工作目錄填 AgentAssistant 倉庫根。',
    mcpArgs: '啟動參數（每行一個）',
    mcpArgsPlaceholder: '-m\nagent_assistant.hosted.mcp_server',
    mcpArgsHint: '原始碼方式兩行：-m 與 agent_assistant.hosted.mcp_server。',
    mcpCwd: '工作目錄',
    mcpCwdPlaceholder: '例如 C:\\Users\\hp\\Desktop\\AgentAssistant',
    mcpDisabledHint: '開啟後會下載並啟動官方 AgentEngine（約 60MB），把它的工具註冊給雲端角色。危險操作會先彈窗確認。',
    mcpDisabledHintLocal: '開啟後會用上面填寫的 Python 啟動本機 AgentAssistant 原始碼。改完原始碼後關掉再開啟電腦操控即可測。',
    mcpStateRunning: '運行中',
