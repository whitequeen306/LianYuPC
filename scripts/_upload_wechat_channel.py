#!/usr/bin/env python3
"""Upload WechatChannel zip + wechat-channel-latest.yml to MinIO updates/.
...
def cleanup_old_channel_zips(client: paramiko.SSHClient, keep: int = RETENTION_RELEASES) -> None:
    list_cmd = (
        "docker exec lianyu-minio sh -lc '"
        "mc alias set local http://127.0.0.1:9000 \"$MINIO_ROOT_USER\" \"$MINIO_ROOT_PASSWORD\" >/dev/null && "
    ...
    for line in out.splitlines():
        ...
        match = CHANNEL_ZIP_RE.fullmatch(filename)
        if not match:
            continue
...
def main() -> None:
    ...
    parser.add_argument("--zip", help="Local WechatChannel zip (default: artifacts/wechat-channel/WechatChannel-win-x64-<version>.zip)")
    parser.add_argument("--version", default="0.1.0", help="Channel semver x.y.z")
    ...
    with tempfile.TemporaryDirectory(prefix="lianyu-wechat-channel-") as tmp:
        manifest_path = Path(tmp) / "wechat-channel-latest.yml"
        write_manifest(manifest_path, version=args.version, filename=filename, sha256=digest, size=size)

        client = connect(password)
        remote_tmp = f"/tmp/lianyu-wechat-channel-{args.version}"
        run(client, f"rm -rf {remote_tmp} && mkdir -p {remote_tmp}")
        sftp_put(client, zip_path, f"{remote_tmp}/{filename}")
        sftp_put(client, manifest_path, f"{remote_tmp}/wechat-channel-latest.yml")

        run(client, "docker exec lianyu-minio sh -lc 'rm -rf /tmp/lianyu-wechat-channel && mkdir -p /tmp/lianyu-wechat-channel'")
        run(client, f"docker cp {remote_tmp}/{filename} lianyu-minio:/tmp/lianyu-wechat-channel/{filename}", timeout=600)
        mc_cp(client, f"/tmp/lianyu-wechat-channel/{filename}", filename)
        run(client, f"docker cp {remote_tmp}/wechat-channel-latest.yml lianyu-minio:/tmp/lianyu-wechat-channel/wechat-channel-latest.yml", timeout=120)
        mc_cp(client, "/tmp/lianyu-wechat-channel/wechat-channel-latest.yml", "wechat-channel-latest.yml", timeout=120)

        run(client, "docker exec lianyu-minio sh -lc 'rm -rf /tmp/lianyu-wechat-channel'")
        run(client, f"rm -rf {remote_tmp}")
        configure_update_assets_public_read(client)
        cleanup_old_channel_zips(client)
        run(
            client,
            "curl -k -fsS -o /dev/null -w 'wechat-channel-latest=%{http_code}' "
            "https://156.233.228.18/api/public/files/updates/wechat-channel-latest.yml",
        )
        client.close()

    print("WECHAT_CHANNEL_UPLOADED")
